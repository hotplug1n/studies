# ✅ TryHackMe — Rooms concluídas

> **Evidência:** capturas do perfil do usuário fornecidas nesta conversa. O perfil exibido informa **78 completed rooms**; as capturas permitem identificar **76 rooms** individualmente. As 2 restantes não aparecem nas capturas fornecidas e não são inferidas.

## Pre Security / fundamentos

- ✅ KoffeeSec - SoMeSINT
- ✅ Search Skills
- ✅ Computer Types
- ✅ Careers in Cyber
- ✅ Inside a Computer System
- ✅ Data Representation
- ✅ Data Encoding
- ✅ Python: Simple Demo
- ✅ JavaScript: Simple Demo
- ✅ Cloud Computing Fundamentals
- ✅ Virtualisation Basics
- ✅ Client-Server Basics
- ✅ What is Networking?
- ✅ Intro to LAN
- ✅ Security Principles

## Linux / Windows

- ✅ Linux Fundamentals Pt1
- ✅ Linux Fundamentals Part 2
- ✅ Linux Fundamentals Part 3
- ✅ Linux CLI Basics
- ✅ Linux Shells
- ✅ Windows Basics
- ✅ Windows Fundamentals 1
- ✅ Windows Fundamentals 2
- ✅ Windows Fundamentals 3
- ✅ Windows Command Line
- ✅ Windows PowerShell
- ✅ Windows CLI Basics
- ✅ Operating Systems: Introduction
- ✅ Operating System Security
- ✅ Active Directory Basics

## Networking / traffic analysis

- ✅ Networking Concepts
- ✅ Networking Essentials
- ✅ Networking Core Protocols
- ✅ Networking Secure Protocols
- ✅ OSI Model
- ✅ Packets & Frames
- ✅ Extending Your Network
- ✅ DNS in Detail
- ✅ Wireshark: The Basics
- ✅ Nmap: The Basics
- ✅ Tcpdump: The Basics

## Cryptography / password security

- ✅ Cryptography Basics
- ✅ Public Key Cryptography Basics
- ✅ Hashing Basics
- ✅ Cryptography Concepts
- ✅ John the Ripper: The Basics

## Web / application security

- ✅ How Websites Work
- ✅ Putting it all together
- ✅ HTTP in Detail
- ✅ Web Application Basics
- ✅ JavaScript Essentials
- ✅ Database SQL Basics
- ✅ SQL Fundamentals
- ✅ Burp Suite: The Basics

## Cybersecurity / blue team / SOC

- ✅ Become a Hacker
- ✅ Defensive Security Intro
- ✅ Become a Defender
- ✅ SOC Role in Blue Team
- ✅ SOC Fundamentals
- ✅ Junior Security Analyst Intro
- ✅ Introduction to SIEM
- ✅ Splunk: The Basics
- ✅ Incident Response Fundamentals
- ✅ Digital Forensics Fundamentals
- ✅ AI Threat Modelling Assessment
- ✅ Training Impact on Teams

## Exploitation / Metasploit / labs

- ✅ Metasploit: Introduction
- ✅ Metasploit: Exploitation
- ✅ Metasploit: Meterpreter
- ✅ Blue
- ✅ Moniker Link (CVE-2024-21413)
- ✅ The Brochure
- ✅ Room 404
- ✅ Packed Light
- ✅ Beach Bar
- ✅ Complimentary

## Integridade da documentação

Este arquivo registra **somente a evidência de conclusão da room**, não substitui anotações pessoais. A presença de uma room aqui não significa que cada conceito foi dominado. As notas em `tryhackme/<room>/README.md` devem diferenciar o que foi comprovadamente estudado do conteúdo de referência pesquisado.

As questões, flags e respostas das rooms não são reproduzidas aqui.
