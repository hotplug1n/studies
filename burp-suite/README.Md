...
## Fluxo recomendado
Configurar proxy → definir escopo → navegar na aplicação → mapear endpoints → reproduzir requisições importantes → testar hipóteses → registrar evidências.

## Comandos / ações úteis
Burp é predominantemente GUI. Para reproduzir uma request fora dele, exportar a requisição e usar ferramentas como `curl` em laboratório.
...
## Status pessoal
**Estudado / em andamento.** O uso e os conceitos do Burp Suite já aparecem no histórico dos estudos; conclusão da room não é presumida.
